...
def writeReading(slave, func_code, starting_address, total_size):
    ...
    slave.startCommunication()
    slave.serial.write(msg_bytearray)
    print(msg_bytearray)


class Slave:
    def __init__(self, slaveID, port='COM1', baudrate=9600, parity=serial.PARITY_NONE, byteSize=8, stopBits=1):
...
class Master:
    ...
    def addSlave(self, slave):
        if slave.getSlaveID() not in self._slaves.keys():
            ...
            slave.serial.reset_input_buffer()
            slave.serial.reset_output_buffer()
    ...
    def readCoils(self, slaveID, starting_address, total_size):
        ...
        writeReading(slave, 0x01, starting_address, total_size)
        no_of_bytes = math.ceil(total_size / 8)
        responseFrameSize = 5 + no_of_bytes
        resp_coil_array = []
        resp_bytearray = slave.serial.read(responseFrameSize)
        print(resp_bytearray)
        slave.serial.flush()
    ...
    def readHoldingRegisters(self, slaveID, starting_address, total_size):
        slave = self._slaves[slaveID]
        writeReading(slave, 0x03, starting_address, total_size)
    ...
    def forceSingleCoil(self, slaveID, address, value):
        slave = self._slaves[slaveID]
        return writeReading(slave, 0x05, address, value)

    # FC06
    def presetSingleRegister(self, slaveID, address, value):
        slave = self._slaves[slaveID]
        return writeReading(slave, 0x06, address, value)
    ...
    # FC16
    def presetMultipleRegisters(self):
        return
